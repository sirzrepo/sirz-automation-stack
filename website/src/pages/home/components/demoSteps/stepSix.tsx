...
const agenciesMarkets = [
    "Yes",
    "No",
];

export default function StepSix({ step, setStep, value, setValue }: { step: number; setStep: (step: number) => void; value: string; setValue: (value: string) => void }) {
  const [isAgency, setIsAgency] = useState<string | null>(value);
  return (
    <div className="w-full max-w-4xl sm:p-16 max-sm:py-16 max-sm:px-6 text-center max-sm:max-h-[85vh] overflow-y-auto bg-white border-r-[0.6em] border-l-[0.6em] border-b-[0.2em] border-teal-400  shadow-2xl border-0 relative rounded-3xl">
        ...
        <h1 className="sm:text-4xl text-2xl font-semibold text-gray-800 mb-8 text-balance">
          Do you currently work with agencies or freelancers?
        </h1>

        {/* Business type selection grid */}
        <div className="mb-8 grid grid-cols-2 gap-6">
          {agenciesMarkets.map((agencyMarket) => {
            const isSelected = isAgency === agencyMarket;
            return (
              <button
                key={agencyMarket}
                ...
                `}
                onClick={() => setIsAgency(agencyMarket)}
                type="button"
        ...
        <div className="max-w-[20em] mx-auto">
          <Button
            type="inverted"
            text="Next"
            onClick={() => {setStep(step + 1); setValue(isAgency || "")}}
