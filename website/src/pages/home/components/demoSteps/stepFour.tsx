...
export default function StepFour({ step, setStep, value, setValue }: { step: number; setStep: (step: number) => void; value: string; setValue: (value: string) => void }) {
  const [selectedTargetMarket, setSelectedTargetMarket] = useState<string | null>(value);
  return (
    <div className="w-full max-w-4xl p-16 text-center bg-white border-r-[0.6em] border-l-[0.6em] border-b-[0.2em] border-teal-400  shadow-2xl border-0 relative rounded-3xl">
        ...
        <div className="mb-8 grid grid-cols-3 gap-6">
          {targetMarkets.map((targetMarket) => {
            const isSelected = selectedTargetMarket === targetMarket;
            return (
              <button
                key={targetMarket}
                ...
                `}
                onClick={() => setSelectedTargetMarket(targetMarket)}
                type="button"
