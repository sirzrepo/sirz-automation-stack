...
export default function BrandcomInquiries() {
  const dispatch = useDispatch();
  const [inquiries, setInquiries] = useState<BrandcomInquiry[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [sortOrder, setSortOrder] = useState<'newest' | 'oldest'>('newest');
  const [selectedInquiry, setSelectedInquiry] = useState<BrandcomInquiry | null>(null);
  ...
  const fetchInquiries = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${BASE_URL}/api/brandcom-form`);
  ...
  const handleDeleteClick = (inquiry: BrandcomInquiry) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/brandcom-form/${selectedInquiry._id}`);
  ...
  const handleView = (inquiry: BrandcomInquiry) => {
    setSelectedInquiry(inquiry);
    ...
    const filteredInquiries = searchQuery
      ? inquiries.filter(
          (inquiry) =>
            ...
            (inquiry.phone && inquiry.phone.toString().includes(searchQuery))
        )
      : inquiries;
  ...
  return (
    <section className="px-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Brandcom Inquiries</h1>
      </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredInquiries.map((inquiry) => (
                  <tr key={inquiry._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{inquiry.company || 'N/A'}</div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900 break-all">{selectedInquiry.email}</p>
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">{selectedInquiry.company || 'N/A'}</p>
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">{selectedInquiry.employees || 'N/A'}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500">Country</h3>
                <p className="mt-1 text-sm text-gray-900">{selectedInquiry.country || 'N/A'}</p>
              </div>
            </div>
            <div>
