...
export default function ChatbotInquiries() {
  ...
  const fetchInquiries = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${BASE_URL}/api/chatbot`);
  ...
  const handleDeleteClick = (inquiry: ChatbotInquiry) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/chatbot/${selectedInquiry._id}`);
  ...
  const handleView = (inquiry: ChatbotInquiry) => {
    setSelectedInquiry(inquiry);
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `chatbot-inquiries-${new Date().toISOString().split('T')[0]}.csv`);
                ...
                {filteredInquiries.map((inquiry) => (
                  <tr key={inquiry._id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{inquiry.firstName}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{inquiry.lastName}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{inquiry.email}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{inquiry.website}</div>
                    </td>
      ...
      >
        <div className="flex flex-col gap-4">
          <p className="text-gray-700">
            Are you sure you want to delete this inquiry by <b>{selectedInquiry?.firstName}</b>?
