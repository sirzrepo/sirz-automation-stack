...
export default function ScheduleDemoInquiries() {
  const dispatch = useDispatch();
  const [inquiries, setInquiries] = useState<ScheduleDemoInquiry[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchQuery, setSearchQuery] = useState('');
  const [sortOrder, setSortOrder] = useState<'newest' | 'oldest'>('newest');
  const [selectedInquiry, setSelectedInquiry] = useState<ScheduleDemoInquiry | null>(null);
  ...
  const fetchInquiries = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${BASE_URL}/api/schedule-demo`);
      setInquiries(response.data);
    } catch (error) {
      console.error('Error fetching schedule demo inquiries:', error);
      toast.error('Failed to fetch schedule demo inquiries');
    } finally {
      setLoading(false);
    }
  };

  const handleDeleteClick = (inquiry: ScheduleDemoInquiry) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await axios.delete(`${BASE_URL}/api/schedule-demo/${selectedInquiry._id}`);
  ...
  const handleView = (inquiry: ScheduleDemoInquiry) => {
    setSelectedInquiry(inquiry);
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute('download', `schedule-demo-inquiries-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  return (
    <section className="px-6">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredInquiries.map((inquiry) => (
                  <tr key={inquiry._id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">{inquiry.firstName}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{inquiry.lastName}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{inquiry.email}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{inquiry.company}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{inquiry.country}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDateTime(inquiry.createdAt)}</td>
      ...
      <Modal
        id="view_inquiry"
        title="Schedule Demo Inquiry Details"
        btnText="Close"
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">{selectedInquiry.firstName}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500">Last Name</h3>
                <p className="mt-1 text-sm text-gray-900">{selectedInquiry.lastName}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">{selectedInquiry.website}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500">Company</h3>
                <p className="mt-1 text-sm text-gray-900">{selectedInquiry.company}</p>
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500">Employees</h3>
                <p className="mt-1 text-sm text-gray-900">{selectedInquiry.employees}</p>
              </div>
              <div>
      ...
      >
        <div className="flex flex-col gap-4">
          <p className="text-gray-700">
            Are you sure you want to delete this inquiry by <b>{selectedInquiry?.email}</b>?<br />
