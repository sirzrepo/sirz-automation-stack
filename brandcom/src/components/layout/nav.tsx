...
const MenuIcon = ({ isOpen, toggle }: { isOpen: boolean; toggle: () => void }) => (
  ...
  >
    <span className={`block w-6 h-0.5 bg-black transition-all duration-300 ${isOpen ? 'rotate-45 translate-y-1.5' : '-translate-y-1'}`} />
    <span className={`block w-6 h-0.5 bg-black my-1.5 transition-opacity duration-300 ${isOpen ? 'opacity-0' : 'opacity-100'}`} />
    <span className={`block w-6 h-0.5 bg-black transition-all duration-300 ${isOpen ? '-rotate-45 -translate-y-1.5' : 'translate-y-1'}`} />
  </button>
);
...
export default function NavBar() {
    ...
    return (
        <div className="fixed bg-white w-full z-40 bg-colorDefaultLight shadow-md">
                                    ...
                                    <div
                                        className={`whitespace-nowrap px-1 py-2 text-sm font-medium transition-colors duration-200 ${
                                            location.pathname.startsWith(item.action) && item.action !== ""
                                                ? "text-[#4517eb]"
                                                : "text-gray-700 hover:text-[#4517eb]"
                                        } cursor-pointer`}
                                        onClick={() => {
